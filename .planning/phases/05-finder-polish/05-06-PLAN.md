...
<task type="auto">
  <name>Task 2: Enhance status bar with selection info</name>
  <files>src/finder.rs</files>
  <action>
Update the status bar to show selection count when items are selected.

1. Update status bar rendering:
```rust
...
2. Optionally add an icon or visual indicator for the status bar.

The status bar should now dynamically show:
- "X items" when nothing selected
- "Y of X selected" when items are selected
  </action>
  <verify>Run `trunk serve`, select items in Finder, verify status bar shows selection count</verify>
  <done>Status bar shows item count and selection count</done>
</task>
...
- Status bar with item and selection count
  </what-built>
  <how-to-verify>
...
11. Select items - verify status bar shows "X of Y selected"
12. Compare overall appearance to macOS Finder screenshot
...
- REQ-005.2: Toolbar with view buttons and search - COMPLETE
...
- REQ-005.7: Status bar with item count - COMPLETE
